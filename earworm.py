...
def do_work(track, options, max_package=False):
    if not options:
        dur = DEF_DUR
        mlp = MIN_JUMP
        lgh = False
    ...
    try:
        if fce == True:
            raise
        graph = read_graph("analytics/" + mp3 + ".graph.gpkl")
    except:
    ...
    # save graph
    if pkl == True:
        save_graph(graph, "analytics/" + mp3 + ".graph.gpkl")
    if gml == True:
        save_graph(graph, mp3+".graph.gml")
    # single loops
    if sho == True:
        short_add = one_loop(graph, track, mode='shortest')
        if not max_package:
            return short_add
        else:
            short_add = short_add[1]
    if lon == True:
        long_add = one_loop(graph, track, mode='longest')
        if not max_package:
            return long_add
        else:
            long_add = long_add[1]
    ...
    if short_add:
        actions.append(short_add)
    if long_add:
        actions.append(long_add)
    return actions
...
def main():
    ...
    if options.verbose:
        print "Rendering..."
    render(actions, "renders/" + str(name), verbose=verbose)
    return 1
